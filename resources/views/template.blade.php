...
<body>
    <div class="container px-4 mx-auto">
        <header class="felx justify-between items center py-4">
            <div class="flex items-center flex-grow gap-4">
                <a href="{{ route('home') }}">
                    <img src="{{ asset('images/logo.png') }}" alt="logo" class="h-12">
                </a>
                <form action="{{ route('home')}}" method="get" class="flex-grow">
                    <input type="search" name="search"placeholder="Buscar" value="{{ request('search') }}" class="border border-gray-200 rounded py-2 px-4 w-1/2">
                </form>
            </div>
            @auth()
